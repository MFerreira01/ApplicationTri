...
{
	return this->sigComposantesConnexes(this->lireNom(), enregistrementCSV);
}

// signatures forme pour Image_Ndg et Image_Couleur
std::vector<SIGNATURE_Forme> CImageClasse::sigComposantesConnexes(std::string path, bool enregistrementCSV) {
	...
	if (this->lireNbRegions() > 0) {
		...
		//std::cout << "(int)tab.size() : " << (int)tab.size() << std::endl;
		for (int k=0;k<(int)tab.size();k++) {
		...
		for (int num = 1; num <= this->lireNbRegions(); num++) { // optimisable sans souci avec un seul parcours plutot que num
			...
			/*
			double a_coef = tab[num].V1i / tab[num].V1j;
			double b_coef = tab[num].centreGravite_i - a * tab[num].centreGravite_j;

			for (int i = tab[num].rectEnglob_Hi; i <= tab[num].rectEnglob_Bi; i++)
			{
				for (int j = tab[num].rectEnglob_Hj; j <= tab[num].rectEnglob_Bj; j++)
				{
					int x = (j - b_coef) / a_coef;
					if (this->operator()(i, j) == num)
					{
						this->operator()(i, j) = 255;
					}
				}
			}
			*/
		...
		// enregistrement
		if (enregistrementCSV) {
			std::string fichier = "res/" + path + "_SForme.csv";
			std::ofstream f (fichier.c_str());

			if (!f.is_open())
				std::cout << "Impossible d'ouvrir le fichier en ecriture !" << std::endl;
			else {
				f << "Objet; CG_i; CG_j; Surface; codeFreeman ; RE_Hi ; RE_Hj ; RE_Bi ; RE_Bj ; perimetre ; lambda1 ; lambda2 ; V1i ; V1j ; V2i ; V2j" << std::endl;
				for (int k = 1; k < (int)tab.size(); k++)
					f << k << ";" << tab[k].centreGravite_i << ";" << tab[k].centreGravite_j << ";" << tab[k].surface << ";" << tab[k].codeFreeman << " ; " << tab[k].rectEnglob_Hi << " ; " << tab[k].rectEnglob_Hj << " ; " << tab[k].rectEnglob_Bi << " ; " << tab[k].rectEnglob_Bj << " ; " << tab[k].perimetre << " ; " << tab[k].lambda1 << " ; " << tab[k].lambda2 << " ; " << tab[k].V1i << " ; " << tab[k].V1j << " ; " << tab[k].V2i << " ; " << tab[k].V2j << std::endl;
			}
			f.close();
		}
	}
...
CImageClasse CImageClasse::Voronoi(CImageNdg seuillee) {
	CImageClasse voronoi(*this, "V8");
	CImageDouble imgDouble(seuillee.transformation("complement"));
	CImageDouble distFond = imgDouble.distance("V8", 0);

	int index = 0;
	int valeur = 0;
	bool encontered = true;
	while (encontered) {
		encontered = false;
		index++;
		//std::cout << index << std::endl;

		for (int i = 1; i < distFond.lireHauteur() - 1; i++)
			for (int j = 1; j < distFond.lireLargeur() - 1; j++)
			{
				if (distFond(i, j) == index) // TODO LABE changer faire une image intermédiaire pour chaque pixel changé 
				{
					encontered = true;
					valeur = 0;
					if (voronoi(i - 1, j - 1) != 0) {
						valeur = voronoi(i - 1, j - 1);
					}
					if (voronoi(i, j - 1) != 0) {
						if (valeur != 0 && valeur != voronoi(i, j - 1)) valeur = -1;
						if (valeur != -1) valeur = voronoi(i, j - 1);
					}
					if (voronoi(i + 1, j - 1) != 0) {
						if (valeur != 0 && valeur != voronoi(i + 1, j - 1)) valeur = -1;
						if (valeur != -1) valeur = voronoi(i + 1, j - 1);
					}
					if (voronoi(i - 1, j) != 0) {
						if (valeur != 0 && valeur != voronoi(i - 1, j)) valeur = -1;
						if (valeur != -1) valeur = voronoi(i - 1, j);
					}
					if (voronoi(i + 1, j) != 0) {
						if (valeur != 0 && valeur != voronoi(i + 1, j)) valeur = -1;
						if (valeur != -1) valeur = voronoi(i + 1, j);
					...
					if (voronoi(i, j + 1) != 0) {
						if (valeur != 0 && valeur != voronoi(i, j + 1))
						{
							valeur = -1;
						}
						if (valeur != -1)
						{
							valeur = voronoi(i, j + 1);
						}
					}
					if (voronoi(i + 1, j + 1) != 0) {
						if (valeur != 0 && valeur != voronoi(i + 1, j + 1)) valeur = -1;
					...
					if (valeur == -1) voronoi(i, j) = 0;
					else voronoi(i, j) = valeur;
				}
			}
	}
	return voronoi;
...
CImageNdg CImageClasse::to_ndg(void)
{
	CImageNdg image_ndg(this->lireHauteur(), this->lireLargeur(), 0);
	for (int i = 0; i < this->lireHauteur(); i++)
	{
		for (int j = 0; j < this->lireLargeur(); j++)
		{
