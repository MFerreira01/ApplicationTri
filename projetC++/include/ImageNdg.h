...
public:
	...
	// Déclaration de la fonction pour créer un histogramme à partir d'un tableau de pixels
	__declspec(dllexport) void* creerHistoApartirTableau(int hauteur, int largeur, unsigned char* pixels, bool enregistrementCSV);

	// histogramme
	_declspec(dllexport) std::vector<unsigned long> histogramme(bool enregistrementCSV);
