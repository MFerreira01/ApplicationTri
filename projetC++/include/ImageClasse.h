...
class CImageClasse {
	...
	///////////////////////////////////////
		...
		_declspec(dllexport) std::vector<SIGNATURE_Forme> sigComposantesConnexes(std::string path, bool enregistrementCSV = false);

		// morphologie
		_declspec(dllexport) CImageClasse morphologie(const std::string& methode = "erosion", const std::string& eltStructurant = "V8");

		_declspec(dllexport) CImageClasse Voronoi(CImageNdg seuillee);

		_declspec(dllexport) CImageNdg to_ndg(void);
};
